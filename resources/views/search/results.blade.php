    ...
    <div class="py-8 max-w-7xl mx-auto space-y-10">
            ...
            <div class="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-md">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody>
                        ...
                        <tr title="Click to View Details" class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition" onclick="window.location='{{ route('accounts.show', $account->uuid) }}'">
                            <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-100">{{ $account->username ?? "__" }}</td>
                            <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-100">{{ $account->email ?? "--" }}</td>
                            <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-100">{{ $account->phone ?? '--' }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-4">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-gray-200">
                    <tr>
                        <th class="px-4 py-2 text-left text-sm font-medium">Recipient</th>
                        <th class="px-4 py-2 text-left text-sm font-medium">Content</th>
                        <th class="px-4 py-2 text-left text-sm font-medium">Date</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($messages as $message)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition" onclick="window.location='{{ route('messages.show', $message->id) }}'">
                        <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-100">{{ $message->recipient }}</td>
                        <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-100">{{ Str::limit($message->content, 50) }}</td>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-4">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition" onclick="window.location='{{ route('locations.show', $location->id) }}'">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-4">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition" onclick="window.location='{{ route('credentials.show', $credential->id) }}'">
                        <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-100">{{ $credential->site }}</td>
